...
<body>
	...
	<small style="color:red">(영문일 경우 파일명 뒤에  _en, 한글일 경우 파일명 뒤에 _ko추가)</small><br><!-- 국가 코드 설정 -->
...
<span>- 리소스 번들안에 존재하며 번들안의 메세지를 읽어오는 태그</span><hr>
	&lt;fmt:message key="properties파일안의 변수이름" var="메세지를 저장하는 변수이름" scope="4개영역" /&gt;<br><!-- scope : default 는 request -->
</pre>
	<fmt:bundle basename="com.member">
		...
		<!-- <fmt:message key="name" var="msg" />는 <fmt:message key="name"/> =msg 라고 선언을하는것
		 변수로 만든 것이다. msg= message key="name"라는 뜻--> var msg= (name 을 key 값 으로 가지고 있는 value의  값 )
</body>
</html>